"""
Program: finalProj.py
Programmers: Chase Dickerson and Jacob Schaum
Description: This program creates a decision tree based off the data set finalProj.csv
"""
...
def preprocessData():
    with open('nflAllDowns1.csv', "r") as file:
        for line in itertools.islice(file, 1, 6700):
            buy_computer = {}
            buy_computer["qtr"] = line.split(',')[1]
            buy_computer["down"] = line.split(',')[2]
            buy_computer["Redzone"] = line.split(',')[3]
            buy_computer["ydstogo"] = line.split(',')[4]
            buy_computer["ScoreDiff"] = line.split(',')[5]
            training_data.append(tuple((buy_computer, line.split(',')[6].strip())))
